"""
eval_mdm_lego_by_textlength.py
───────────────────────────────
实验目的：验证 LeGO 相对于 MDM，在 short / medium / long 三种不同文本长度下，
         FID 和 R_precision 是否都有提升。

边界（由 eval_mdm_lego_histogram.py 的 33%/67% 分位数确定）：
    Short  ≤ 8 words   (1548 条, 35.3%)
    Medium 9-13 words   (1360 条, 31.0%)
    Long   ≥ 14 words   (1476 条, 33.7%)

用法:
    python eval_mdm_lego_by_textlength.py              # 使用缓存
    python eval_mdm_lego_by_textlength.py --no_cache   # 忽略缓存，强制重新生成
"""
...
# 由 eval_mdm_lego_histogram.py 确定的 33%/67% 分位数边界
SHORT_UPPER = 8    # Short : ≤ 8 words   (1548 条, 35.3%)
LONG_LOWER  = 14   # Long  : ≥ 14 words  (1476 条, 33.7%)
# Medium: 9 – 13 words                    (1360 条, 31.0%)
...
def compute_text_lengths():
    """
    读取测试集所有文本文件的第一行 caption，统计单词数量。
    返回 {motion_id: word_count} 和长度列表。
    """
    ...
    print(f'[文本长度] 共 {len(lengths)} 个有效样本, '
          f'min={min(lengths)}, max={max(lengths)}, '
          f'mean={np.mean(lengths):.1f}, median={np.median(lengths):.1f}')
    return id_to_length


def categorize_ids(id_to_length):
    """
    根据硬编码的边界 (SHORT_UPPER=8, LONG_LOWER=14) 将 motion ID 分为三组。
    """
    short_ids, medium_ids, long_ids = [], [], []

    for mid, wc in id_to_length.items():
        if wc <= SHORT_UPPER:
    ...
    total = len(id_to_length)
    print(f'[分类结果] Short (≤{SHORT_UPPER}): {len(short_ids)} 条 ({len(short_ids)/total*100:.1f}%)')
    print(f'[分类结果] Medium ({SHORT_UPPER+1}-{LONG_LOWER-1}): {len(medium_ids)} 条 ({len(medium_ids)/total*100:.1f}%)')
    print(f'[分类结果] Long (≥{LONG_LOWER}): {len(long_ids)} 条 ({len(long_ids)/total*100:.1f}%)')
    return short_ids, medium_ids, long_ids
...
def create_filtered_dataloader(args, category_name, id_list, mode='eval'):
    ...
    """
    temp_split_path = os.path.join('/tmp', f't2m_test_bylen_{category_name}.txt')
...
def main():
    ...
    print('#  1. 文本长度分类')
    print('#' * 60)
    print(f'  边界: Short ≤ {SHORT_UPPER}, Medium {SHORT_UPPER+1}-{LONG_LOWER-1}, Long ≥ {LONG_LOWER}')

    id_to_length = compute_text_lengths()
    short_ids, medium_ids, long_ids = categorize_ids(id_to_length)
