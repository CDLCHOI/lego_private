...
def evaluate_matching_score(eval_wrapper, motion_loaders, file):
    ...
    return match_score_dict, R_precision_dict, activation_dict, match_score_sim_dict, R_precision_sim_dict


def evaluate_fid(eval_wrapper, groundtruth_loader, activation_dict, file):
    '''
    groundtruth_loader: 最下面mode='gt'的那个
    '''
...
def evaluation(eval_wrapper, gt_loader, eval_motion_loaders, log_file, replication_times=1, diversity_times=300, mm_num_times=0, run_mm=False):
    with open(log_file, 'a+') as f:
        ...
        for replication in range(replication_times):
            ...
            print(f'Time: {datetime.now()}')
            print(f'Time: {datetime.now()}', file=f, flush=True)
            mat_score_dict, R_precision_dict, acti_dict, mat_score_sim_dict, R_precision_sim_dict = evaluate_matching_score(eval_wrapper, motion_loaders, f)
...
if __name__ == '__main__':
    ...
    log_file = f"{os.path.dirname(args.resume_trans)}/Merged"
    ...
    if 'best_diff' in args.resume_trans:
        log_file += '_best_diff'
    elif 'last' in args.resume_trans:
        log_file += '_net_last'
    elif 'best' in args.resume_trans:
        log_file += '_net_best'
    if args.evaluator_eval == 'MARDM':
        log_file += '_MARDM'
    log_file += '.log'
    ...
    logger.info(f'args.add_clip_lora = {args.add_clip_lora}')
    ...
    load_ckpt(net, args.resume_trans, key='base', strict=False)
    merge_clip = torch.load(os.path.join(os.path.dirname(args.resume_trans), 'merged_clip.pth'))
    missing_keys, unexpected_keys = net.clip_model.load_state_dict(merge_clip, strict=True)
