        ...
        for metric_name, metric_dict in all_metrics.items(): # odict_keys(['Matching Score', 'R_precision', 'FID', 'Diversity', 'MultiModality', 'Control_l2', 'Skating Ratio', 'Trajectory Error'])
            ...
            for model_name, values in metric_dict.items():
                ...
                if isinstance(mean, np.float64) or isinstance(mean, np.float32):
                    ...
                    raise NotImplementedError(f'Not implemented for metric {metric_name}')
        return mean_dict
...
if __name__ == '__main__':
    ...
    # ipdb.set_trace()
    load_lora_mdm_for_eval(net, args.resume_trans, args.add_clip_lora, logger=logger)
    ...
    #评估生成数据集部分  
    shuffle = False
    gt_loader = dataset_control.DataLoader(batch_size=args.batch_size, args=args, mode='gt', split='test', shuffle=shuffle, num_workers=0, drop_last=True)
    gen_loader = dataset_control.DataLoader(batch_size=args.batch_size, args=args, mode='eval', split='test', shuffle=shuffle, num_workers=0, drop_last=True)
    eval_motion_loaders = {
        ## HumanML3D Dataset##
