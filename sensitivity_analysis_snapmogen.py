...
def test(clip_version, batch_size, max_samples):
    ...
    sim_array_filename = f'sensitivity_sim_array_{args.max_samples}_{"test" if args.only_test_set else "all"}_snapmogen.txt'
    sample_text_filename = f'sensitivity_sample_text_{args.max_samples}_{"test" if args.only_test_set else "all"}_snapmogen.txt'
    count = [0] * 10 # 10个区间
    ...
    # 绘制3D相似度柱状图
    plot_3d_similarity_histogram(sim_array, save_path=f"sensitivity_histogram_3d_{args.max_samples}_{'test' if args.only_test_set else 'all'}_snapmogen.png")
