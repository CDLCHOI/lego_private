...
class GaussianDiffusion:
    ...
    def masked_l2(self, a, b, mask):
        ...
        loss = self.l2_loss(a, b) # (b,263,1,196)
        loss = sum_flat(loss * mask.float())  # (b,)  即这行相当于 loss=loss.sum(1,2,3)
        n_entries = a.shape[1] * a.shape[2] # 263
        non_zero_elements = sum_flat(mask) * n_entries # (b,)  每个元素为每个motion里实际的帧数*263
    ...
    def gradients(self, x, hint, mask_hint, joint_ids=None):
        with torch.enable_grad():
            ...
            x_ = x_.squeeze(2)
            dim = x_.shape[-1]
            x_ = x_ * self.std[:dim] + self.mean[:dim]
            n_joints = 22 if x_.shape[-1] in [263, 67] else 21
    ...
    def guide(self, x, t, model_kwargs=None, t_stopgrad=-10, scale=.5, n_guide_steps=10, train=False, min_variance=0.01):
        ...
        Spatial guidance
        """
        n_joint = 22 if x.shape[1] in [263, 67] else 21
    ...
    def p_sample(
        ...
        if 'hint' in model_kwargs['y'].keys():
            ...
            out['mean'] = self.guide(out['mean'], t, model_kwargs=model_kwargs) # 均值引导
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None, dataset=None):
        ...
        model_output = model(x_t, self._scale_timesteps(t), **model_kwargs) # 这里执行网络主体的前向
