...
def get_paired_text():
    ...
    print('num of paired texts: ', len(paired_texts))
...
# def visualize_tsne(paired_texts, clip_model, title, save_path, emb):
    
#     print(f"Plotting t-SNE for {emb.shape[0]//2} pairs ...")
#     all_embeddings = emb
    
#     # t-SNE 降维
#     # perplexity 设小一点因为样本少，init='pca' 保持结构
...
#     # 区分两类点：Sentence A (Original) 和 Sentence B (Modified)
#     # 偶数索引是 c1, 奇数索引是 c2
#     points_a = embeddings_2d[0::2]
#     points_b = embeddings_2d[1::2]
    
#     # 画点
#     plt.scatter(points_a[:, 0], points_a[:, 1], c='blue', alpha=0.6, label='Original (e.g., Left)', s=4)
#     plt.scatter(points_b[:, 0], points_b[:, 1], c='red', alpha=0.6, label='Modified (e.g., Right)', s=4)
    
#     # 画线连接每一对
#     # for i in range(len(points_a)):
#     #     plt.plot([points_a[i, 0], points_b[i, 0]], 
